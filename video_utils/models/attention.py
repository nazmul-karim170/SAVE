...
from diffusers.models.attention import CrossAttention as CrossAttention_dim #, FeedForward, AdaLayerNorm
from .cross_attention import CrossAttention, FeedForward, AdaLayerNorm
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        # Temp-Attn
        self.attn_temp = CrossAttention_dim(
            query_dim=dim,
            heads=num_attention_heads,
            dim_head=attention_head_dim,
            dropout=dropout,
...
class FrameAttention(CrossAttention):
    ...
    #     # ## UMAR Attention
    #     # select_1 = 0
    #     # select_2 = video_length - 1  # video_length//2
    #     #
    #     # key = rearrange(key, "(b f) d c -> b f d c", f=video_length)
    #     # key = torch.cat([key[:, [select_1] * video_length], key[:, [select_2] * video_length]], dim=2)
    #     # key = rearrange(key, "b f d c -> (b f) d c")
    #     #
    #     # value = rearrange(value, "(b f) d c -> b f d c", f=video_length)
    #     # value = torch.cat([value[:, [select_1] * video_length], value[:, [select_2] * video_length]], dim=2)
    #     # value = rearrange(value, "b f d c -> (b f) d c")
    #
    #     # Sparse-Causal
    #     former_frame_index = torch.arange(video_length) - 1
    ...
    #         hidden_states = xformers.ops.memory_efficient_attention(query, key, value, scale=self.scale)
    #
